...
|----------|-------------|
| **Trip and project attribution is by non-exclusive tag** | Corrected 2026-09-13: this row previously described *line-item* actuals, which do not exist — PROJECTS-BRD §5.5.5 puts linking a line item to a transaction out of scope, so line items carry an estimate and nothing else. The real non-additivity is tag overlap. A transaction's `tags` is an array, so one charge can count fully toward a trip *and* a project, or toward two projects. Consequently: a tool must never emit a total that sums across trips or projects, must carry an explicit machine-readable flag saying so on every payload that reports tag-derived money, and must not invite reconciliation against household spending for the period — tagged transactions count even when dated outside the entity's window (PROJECTS-BRD §6). Within a single project the per-category figures *do* reconcile to its total, and that total is safe to state. |
| **Project line items are estimates, not actuals** | `estimatedCost` is a plan. No tool may present a line item as money spent, because the app performs no such reconciliation. |
