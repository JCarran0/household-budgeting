...
>
> **Status as of 2026-08-03: 7 of 30 closed** (`SA-01` dependencies, `SA-03` cost-cap attribution, `SA-11` trust proxy, `SA-12` lockout casing, `SA-19` Plaid tokens off the wire, `SA-24` admin gate, `SA-30` vestigial encryption key).
>
> **`SA-25` — the last High — is code complete but ⛔ blocked on two IAM grants** that need production credentials. Secrets move to SSM Parameter Store; the switch commit is held unpushed until the grants land, because pushing it triggers a deploy that fails its own pre-flight. Ready-to-run commands are in the [SA-25 burn-down section](SECURITY-AUDIT-2026-08-02.md#-needs-production-credentials--sa-25-is-blocked-here). This also supersedes **TD-025**: SSM becomes a durable readable copy of `PLAID_ENCRYPTION_SECRET`, so losing the EC2 instance no longer orphans the Plaid tokens.
>
> The largest open *code* item is **`SA-10`** (open registration chaining into admin escalation). The trivial tier is done.
>
...
**Status**: **Resolved 2026-09-12** — both guards shipped with regression tests; the reconciler `status` refresh went with them
**Created**: 2026-09-12
...
1. `PlaidLinkProvider` now tracks what the loaded token was minted for in a `loadedRef` (`{mode: 'create'} | {mode: 'update', accountId}`), set when the token is requested and cleared wherever `openRef` is. `openPlaid` reuses only a create-mode instance; `openPlaidUpdate` only an update-mode instance **for that same account**; anything else drops the loaded instance and mints a fresh token. `handleSuccess` branches on the ref rather than on `updateAccountId` state — the mode a token was created with is authoritative at callback time, which render state is not. The `updateAccountId` state is gone. `PlaidLinkComponent` is keyed on the token so a new one always mounts a fresh Link instance.
2. `connectAccount` refuses a link whose masked account is already live under a different Item, naming the connected account and pointing at "Sign in to Bank". It refuses the *whole* link rather than storing the non-colliding subset, and releases the Item it just exchanged via `plaidService.removeItem` so a refusal cannot leave the orphan Item this incident left behind. Null masks are exempt — Venmo and some investment profiles report none, and this family legitimately holds two such profiles on separate Items.
3. Item identity in the UI was **not** done. It is a display nicety once linking a duplicate is impossible, and the accounts page is already dense.

Three further defects surfaced only by exercising the fix against the Plaid **sandbox**, none of which the unit tests could have caught:

4. **The refusal reported as HTTP 500.** `routes/accounts.ts` answered `res.status(500)` for every `success: false` from `connectAccount`, so the guard's specific, actionable message rendered as "Connection Error — Request failed with status code 500". `ConnectAccountResult` now carries `code: 'DUPLICATE_INSTITUTION'` and the route answers **409** for it.
5. **The provider discarded every server message.** Both mutation `onError` handlers read `error.message`, which on an axios failure is the literal string "Request failed with status code NNN" — the server's explanation is in the response body. They now use `getApiErrorMessage`, the helper this repo already had, whose own docblock says "do not reinvent the cast chain". This was not new: *every* connect and re-auth failure has shown a status code instead of a reason for as long as the code has existed.
6. **A failed mutation left the Link instance loaded.** Neither `onError` reset `token`/`openRef`/`loadedRef`, so a create-mode instance outlived the click that made it and was there to be inherited by the next "Sign in to Bank". This is the *enabling* half of the incident — the mode guard makes inheriting it harmless, but the instance should not survive at all. Both handlers now call `resetLink()`.

**Verified live against the Plaid sandbox** (2026-09-12), which is the only way items 4-6 were going to be found. The sequence, from the backend log: a refused duplicate link at 12:37:48 (poisoning the state with a loaded create-mode instance), then `POST /accounts/{id}/link-token` at 12:38:02 — the update-mode token request that pre-fix never happens — then `POST /accounts/{id}/reauth-complete` at 12:38:20, proving `handleSuccess` routed to re-auth rather than `connectAccount`. The institution ended with one Item and 14 accounts, not two and 28.

Also fixed on the same path, though it belongs to TD-020's reconciler: `reconcile-plaid-account-change.ts` now refreshes `status`/`pending`/`date`/`amount`/`name`/`merchantName` from the live Plaid row when it re-keys, not just the ids, and the dry run prints a `status changes` section. Previously a row that was `pending` when the gap opened stayed pending forever — the cursor consumed the pending→posted update and Plaid never re-sends it — and `isSkippable` drops pending rows from every budget and report. That is how a $6,131 paycheck stayed invisible for two weeks.

Both guards have regression tests that were **run against the pre-fix code and confirmed to fail** (`duplicate-item-link.test.ts`: 2 of 6 fail without the backend guard; `PlaidLinkProvider.test.tsx`: 3 of 4 fail without the frontend guard, with the legitimate-reuse case passing in both states).
...
- `backend/src/routes/accounts.ts` (409 for a refused duplicate)
- `frontend/src/lib/api/errors.ts` consumers — `PlaidLinkProvider` now uses `getApiErrorMessage`
- `backend/src/scripts/reconcile-plaid-account-change.ts` (re-key now refreshes mutable fields)
- `backend/src/__tests__/critical/duplicate-item-link.test.ts`, `frontend/src/providers/PlaidLinkProvider.test.tsx` (added)

---

**Last Updated**: 2026-09-12 (TD-028 added and resolved)
