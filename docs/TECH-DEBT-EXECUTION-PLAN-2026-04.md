...
6. **`MantineAccounts.tsx` (688 LOC)** — last remaining frontend page over the 600 LOC ceiling. Original TD-010 companions `Budgets.tsx` and `MantineDashboard.tsx` are no longer targets: the 2026-04-23 legacy-tab retirement (commit `0d909aa`) dropped `Budgets.tsx` to 236 LOC, and `MantineDashboard.tsx` is 481 LOC (both under threshold).
