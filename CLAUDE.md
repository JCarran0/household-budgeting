...
- `shared/utils/projectHelpers.ts` — project tag generation, status derivation, line item tag slugs, and `computeLineItemSpending` (tag→line-item attribution + unattributed remainder)

**Two landmines to know about:**
...
- **Project/trip line item actuals are not additive.** A transaction carrying two line item tags counts fully toward both (BRD §5.5.5), so `Σ(line item actuals)` can exceed `totalSpent`. Never render a total for that column — show *Unattributed* instead. `transactionFilterEngine.ts` also has **no `isSplit`/`parentTransactionId` awareness**; correctness relies on split parents being `isHidden` and children inheriting tags.
...
| A route starts rejecting every valid body after a chat action imports its schema | Import cycle: `services/index -> chatActions -> routes/X -> services/index`. The route's Zod schema resolves to **undefined** at module-eval time, so `schema.parse` throws and every request 400s — with a `TypeError: Cannot read properties of undefined` downstream rather than anything naming the cycle. Route-owned Zod schemas therefore live in `backend/src/validators/`, and BOTH the route and the action import from there. `routeImportCycles.test.ts` fails if any non-route module imports a route module |
| A `shared/types` change builds locally but fails CI at `tsc -b` | `npx tsc --noEmit` in `frontend/` does NOT resolve the shared project; `tsc -b` (what `npm run build` and `npm run lint:all` run) does. Verify the frontend with **`npm run lint:all`**, never bare `tsc --noEmit`. Widening a union in `shared/types` commonly breaks an exhaustive `Record<Union, T>` on the frontend — this cost six consecutive failed production deploys on 2026-09-13. The pre-commit hook now runs the frontend gate on `shared/` changes too |
| "Why did the assistant do that?" — a chat action failed, a card went stale, a turn cost more than it should | `npx tsx backend/scripts/inspect-traces.ts --prod --family <id> --since 3h` prints the trace ledger beside the proposal timeline; `--trace <id>` dumps the tool results the model actually saw. A proposal whose trace is an **errored** one is a turn that outlived its request |
...
| 2026-09-12 | Projects line items v2 — line items move from nested-in-`ProjectCategoryBudget` to **project-level** with a required match `tag` and no `categoryId`; category budgets (budgeting axis) and line items (PM axis) are **siblings, not nested**; actual = Σ project-tagged txns carrying the item's tag, deliberately naive (multi-tag double-counts, so the actuals column is never summed); adds *Unattributed* spend; allocation hint re-anchored to `totalBudget`. Fixes two live bugs: project/trip summaries counted **hidden** transactions (`includeHidden: true`), and split children did not inherit the parent's tags | [PROJECTS-BRD.md](docs/features/PROJECTS-BRD.md) §5.5–5.6 |
