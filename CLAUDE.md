...
| 2026-04 | Trip Place Photos — Google Places thumbnails on verified stops | PR 1: `photoName` + `photoAttribution` captured on `VerifiedLocation` at place selection (migrated `LocationInput` from legacy `PlacesService.getDetails` to `Place.fetchFields`); Stay banner swaps `IconBed` for a 48px thumbnail, Stay form shows a 96px preview. PR 2 (deferred, gated on PR 1 real-usage quality) lights up Eat/Play stop cards + Map popup. No backend service change; Zod `verifiedLocationSchema` extended with two optional string fields so they survive validation. No backfill — release note documents re-select pathway. Attribution satisfied by hover tooltip (D6). See [TRIP-PLACE-PHOTOS-BRD.md](docs/features/TRIP-PLACE-PHOTOS-BRD.md). |
