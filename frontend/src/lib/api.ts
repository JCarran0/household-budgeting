...
class ApiClient {
  ...
  constructor() {
    ...
    this.createUpdateLinkToken = this.createUpdateLinkToken.bind(this);
    this.completeReauth = this.completeReauth.bind(this);
    ...
    this.getActualsOverrides = this.getActualsOverrides.bind(this);
  ...
  async createUpdateLinkToken(accountId: string): Promise<{ link_token: string; expiration: string }> {
    const { data } = await this.client.post(`/accounts/${accountId}/link-token`);
    if (!data.success) {
  ...
  async completeReauth(accountId: string): Promise<void> {
    const { data } = await this.client.post(`/accounts/${accountId}/reauth-complete`);
    if (!data.success) {
  ...
  async getActualsOverrides(): Promise<ActualsOverride[]> {
    try {
      const { data } = await this.client.get('/actuals-overrides');
      if (!data.success) {
  ...
  async getActualsOverride(month: string): Promise<ActualsOverride | null> {
    try {
      const { data } = await this.client.get(`/actuals-overrides/${month}`);
      if (!data.success) {
  ...
  async createOrUpdateActualsOverride(override: CreateActualsOverrideDto): Promise<ActualsOverride> {
    try {
      const { data } = await this.client.post('/actuals-overrides', override);
      if (!data.success) {
  ...
  async deleteActualsOverride(overrideId: string): Promise<void> {
    try {
      const { data } = await this.client.delete(`/actuals-overrides/${overrideId}`);
      if (!data.success) {
