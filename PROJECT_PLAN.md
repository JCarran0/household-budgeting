...
## Budget Tab Retirement ✅ COMPLETE

**Status:** Retired 2026-04-23 (branch `chore/tech-debt-sprint-5`). Soft-hide began 2026-04-21; permanent deletion two days later once both users confirmed BvA II covered their workflows.

The new **Budget vs. Actuals** tab (formerly "Budget vs. Actuals II" — BRD at
`docs/features/BUDGET-VS-ACTUALS-II-BRD.md`) superseded the two original
Budget page tabs:
...
**Retirement tasks completed:**

- [x] Deleted `Tabs.Panel value="budget"` and `Tabs.Panel value="comparison"` from `frontend/src/pages/Budgets.tsx`.
- [x] Deleted the five orphaned components:
  - `frontend/src/components/budgets/BudgetGrid.tsx`
  - `frontend/src/components/budgets/BudgetComparison.tsx`
  - `frontend/src/components/budgets/BudgetSummaryCards.tsx`
  - `frontend/src/components/budgets/BudgetForm.tsx`
  - `frontend/src/components/budgets/BudgetDebugger.tsx`
- [x] Audited `Budgets.tsx` — dropped all unused imports, state, queries, handlers, and the `<BudgetForm>` modal. Page is now ~230 LOC (from 602).
- [x] Audited `frontend/src/lib/api/budgets.ts` — dropped `getBudgetComparison`, `applyRollover`, `copyBudgets`, and the `BudgetComparison` / `BudgetComparisonResponse` types.
- [x] Backend routes deleted: `POST /api/budgets/comparison/:month`, `POST /api/budgets/copy`, `POST /api/budgets/rollover`. Corresponding service methods (`getBudgetComparisonForMonth` deleted; `copyBudgets` / `calculateRollover` / `applyRollover` / `getBudgetVsActual` / `getMonthlyBudgetVsActual` kept as well-tested library code, covered in `financial-calc.stories.test.ts` + `budgetService.test.ts`).
- [x] Integration tests: deleted `backend/src/__tests__/integration/budget-date-filtering.stories.test.ts` (entire file was about the retired endpoint). Removed the one `Subcategories of hidden parents should be excluded from budget calculations` test from `hidden-categories.stories.test.ts` that used the retired endpoint.
- [x] Docs updated — `AI-APPLICATION-ARCHITECTURE.md`, BvA II BRD, `CATEGORY-HIERARCHY-BUDGETING-BRD.md`, `ROLLOVER-BUDGETS-BRD.md`.
